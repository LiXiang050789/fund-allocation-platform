"""
农行杯赛题一 · 指数基金智能配置决策平台 (v2)
Streamlit 前端 — 5 页 + 动态交互 + DeepSeek API
"""
...
with tab3:
    ...
    etf_weights = [equity_pct/5]*5 + [(1-equity_pct)/2]*2  # 权益均分5只, 避险均分2只

    fig_bar = go.Figure()
    colors_bar = ['#E91E63']*5 + ['#FFC107','#4CAF50']
    fig_bar.add_trace(go.Bar(x=etf_names, y=[w*100 for w in etf_weights], marker_color=colors_bar,
                             text=[f'{w*100:.0f}%' for w in etf_weights], textposition='outside'))
    fig_bar.add_hline(y=equity_pct*100, line_dash='dash', line_color='gray',
                      annotation_text=f'权益中枢 {equity_pct*100:.0f}%')
    fig_bar.update_layout(height=350, margin=dict(t=20,b=20), yaxis_title='权重(%)',
                          yaxis=dict(range=[0, max(etf_weights)*100+10]))
