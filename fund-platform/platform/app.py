...
with tab3:
    ...
    captions = {
        "保守型(C2)": "💡 C2 适合退休/低风险投资者。LGB 融合年化 7.90%、回撤仅 -10.26%，在保守档全面优于动态评分。",
        "平衡型(C3)": "💡 C3 是 sweet spot——LGB 融合年化 14.58%/-21.83% 回撤，对大多数零售客户可接受。",
        "进取型(C4)": "💡 C4 年化 15.96% 但回撤 -27.70%，超出多数零售客户通常承受范围（≤20%），谨慎推荐；进取型可另选 SASF 硬切换（年化 16.52%、回撤 -26.56%）。",
    }
...
with tab5:
    ...
    risk_table = pd.DataFrame(
        [
            ...
            ["C2", "谨慎型", "R1、R2（如债券基金）", "21-40 分", "年化 7.9%/回撤 -10.3%"],
            ["C3", "稳健型", "R1、R2、R3（如混合基金）", "41-65 分", "年化 14.6%/回撤 -21.8% ✨"],
            ["C4", "积极型", "R1-R4（如股票基金）", "66-90 分", "年化 16.0%/回撤 -27.7%（SASF 16.5%/-26.6%）"],
