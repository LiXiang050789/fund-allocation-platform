...
更新：PPO文件缺失自动跳过，不中断回测；增加PPO权重clip校验
"""
import pandas as pd
import numpy as np
import warnings
...
# PPO权重输出路径（需提前生成）
PPO_WEIGHT_PATH = r"E:\农行杯\2026\1\weight\ppo_weight.csv"
...
def strategy_ppo_daily(dates):
    """PPO原生日频调仓，文件缺失返回None"""
    if not os.path.exists(PPO_WEIGHT_PATH):
        print(f"⚠️ PPO日频权重文件不存在 {PPO_WEIGHT_PATH}，跳过【PPO原生日频】策略")
        return None
    ppo_daily = pd.read_csv(PPO_WEIGHT_PATH, parse_dates=["date"])
    ppo_daily = ppo_daily.set_index("date")
    ppo_daily = ppo_daily.clip(lower=MIN_WEIGHT, upper=MAX_WEIGHT)
...
print("========== 运行【月度调仓】主实验 ==========")
for name, strat_fn in strategies_monthly:          # ✅ 改为 strategies_monthly
    print(f'\n[{name}] 正在计算权重...')
    ...
    nav, daily_ret, daily_w = run_backtest(name, w_df, dates_arr, prices_mat, returns_mat)  # ✅ 5个参数
    all_navs[name] = nav
...
for name, strat_fn in strategies_daily:
    ...
    nav, daily_ret, daily_w = run_backtest(name, w_df, dates_arr, prices_mat, returns_mat)  # ✅ 补上 prices_mat
    all_navs[name] = nav
    all_weights[name] = daily_w
    m = calc_metrics(name, daily_ret, w_df, nav)
    results.append(m)
