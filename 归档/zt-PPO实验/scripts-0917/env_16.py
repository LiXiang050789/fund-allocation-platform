    ...
    def step(self, action):
        ...
        if self.rebalance_monthly and not self._is_rebalance_day(self.current_idx):
            ...
            if self.enable_equity_only:
                # 权益固定等权；PPO 只学"债券权重相对规则的偏离"
                rule_w = self._rule_baseline_weight()
                rule_bond_w = float(rule_w[BOND_IDX])

                # 用 action 的均值作为债券偏离信号
                # tanh 压缩到 [-1, 1]，再乘 residual_scale
                a_mean = float(np.mean(action))
                a_signal = np.tanh(a_mean)  # [-1, 1]
                bond_w = rule_bond_w + self.residual_scale * a_signal

                # 债券权重约束到 [0.05, 0.70]，与规则一致
                bond_w = float(np.clip(bond_w, 0.05, 0.70))
