"""
农行杯赛题一 · 指数基金智能配置决策平台
Streamlit 前端 — 市场仪表盘 / 策略对比 / 自动化数据流水线 / 配置推荐 / AI助手 / 风险划分
"""
import json
import os
import subprocess
...
def load_data():
    ...
    nav = pd.read_csv(f"{BASE}/results/backtest/all_strategy_nav.csv", encoding="utf-8-sig", parse_dates=["date"])
    seg = pd.read_csv(f"{BASE}/results/backtest/segment_all_strategy.csv", encoding="utf-8-sig")
    tiers = pd.read_csv(f"{BASE}/results/risk_levels_metrics.csv", encoding="utf-8-sig")
...
with tab3:
    ...
    captions = {
        "保守型(C2)": "💡 C2 适合退休/低风险投资者。推荐 LGB 融合：年化 7.90%、夏普 0.626、回撤 -10.26%。",
        "平衡型(C3)": "💡 C3 是 sweet spot——推荐 LGB 融合：年化 14.58%、夏普 0.612、回撤 -21.83%，适合大多数零售客户。",
        "进取型(C4)": "💡 C4 推荐 SASF 硬切换（年化 16.52%、回撤 -26.56%）或 LGB 融合（夏普 0.620），仅向专业投资者推荐。",
    }
